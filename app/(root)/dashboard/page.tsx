...
export default async function DashboardPage() {
  ...
  const completedWithScores = completedSessions.filter(
    (session) => typeof session.finalScore === "number",
  );
  const averageScore =
    completedWithScores.length > 0
      ? Math.round(
          completedWithScores.reduce(
            (total, session) => total + (session.finalScore || 0),
            0,
      ...
      <Tabs defaultValue="active" className="w-full">
        <TabsList className="w-full sm:w-auto">
          <TabsTrigger value="active">
            Practice ({activeSessions.length})
          </TabsTrigger>
